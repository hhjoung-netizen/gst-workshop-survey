...
# [30개 전체 문항 데이터 - 오류 수정 및 철저 검증 완료]
SURVEY_QUESTIONS = [
    ...
    {"no": 8, "문항": "팀 프로젝트에서 가장 잘 맞는 역할은?", "A": "분석 담당", "B": "실행 담당", "C": "소통 담당", "D": "일정/문서 관리 담당"}, # 🔍 오타 수정 완료!
    ...
    {"no": 19, "문항": "협업 시 가장 답답한 상황은?", "A": "논리적이지 못할 때", "B": "행동이 느릴 때", "C": "독단적으로 행동할 때", "D": "規則을 안 지킬 때"},
    ...
    if input_pw == ADMIN_PASSWORD:
        ...
        if df_res.empty:
            ...
            with col2:
                ...
                total_c = df_res["조율형_개수"].sum() if "조율형_개수" in df_res.columns else df_res.iloc[:, 6].sum()
